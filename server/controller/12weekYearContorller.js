...
const { TwelveWeekYear, Goals, User } = require("../model");
...
const getAllRecords = async (req, res) => {
  try {
    ...
    const data = await readService(queryString, TwelveWeekYear, association);
...
const getARecord = async (req, res) => {
  try {
    ...
    const data = await readAService(id, TwelveWeekYear);

    if (!data?.data)
      return handleErrorResponse(res, 404, "Goal not found", "error");
...
const getAuthUserRecords = async (req, res) => {
  try {
    ...
    const data = await readUserServices(
      userID,
      queryString,
      TwelveWeekYear,
      User
...
const create12WeekYear = async (req, res) => {
  try {
    const data = req.body;
...
const updateARecord = async (req, res) => {
  try {
    ...
    const updateDate = await updateService(data, id, TwelveWeekYear, User);
...
const deleteARecord = async (req, res) => {
  try {
    ...
    const deleteDate = await deleteService(data, id, TwelveWeekYear, User);
