...
const { Daily, Task, User } = require("../model");
...
const getAllRecords = async (req, res) => {
  try {
    ...
    const data = await readService(queryString, Task, association);
...
const getARecord = async (req, res) => {
  try {
    ...
    const data = await readAService(id, Task);
...
const getAuthUserRecords = async (req, res) => {
  try {
    ...
    const data = await readUserServices(userID, queryString, Task, User);
...
const updateARecord = async (req, res) => {
  try {
    ...
    const updateDate = await updateService(data, id, Task, User);
...
const deleteARecord = async (req, res) => {
  try {
    ...
    const deleteDate = await deleteService(data, id, Task, User);
    ...
    return res.status(201).json({ ...deleteDate });
  } catch (error) {
    console.log(error);
    return handleErrorResponse(res, 500, "internal server error", "error");
  }
